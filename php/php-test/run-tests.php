...
/* $Id: run-tests.php 283686 2009-07-07 23:05:17Z dsp $ */
...
function verify_config()
{
  ...
  if ($resin_home) {
    if (is_dir($resin_home)) {
      $php = get_quercus_cmd();
      $php_cgi = get_quercus_cgi_cmd();
      return;
    }
    else
      error('invalid resin-home directory');
  }
...
Options:
    --resin-home <dir>     Resin directory
...
function get_quercus_cmd_impl($class_name)
{
  ...
  $classpath = "${resin_home}/lib/resin.jar"
               . "${path_separator}${resin_home}/lib/quercus.jar"
               . "${path_separator}${resin_home}/lib/resin_kernel.jar"
               . "${path_separator}${resin_home}/lib/pro.jar"
               . "${path_separator}${pro_home}/lib/pro.jar"
               . "${path_separator}${pro_home}/lib/license.jar";
  

  if ($_SERVER["JAVA_HOME"])
    $java = "${_SERVER["JAVA_HOME"]}/bin/java";
  else
    $java = "java";
...
";
	...
	if (!empty($section_text['GET']) || !empty($section_text['POST']) || !empty($section_text['POST_RAW']) || !empty($section_text['COOKIE']) || !empty($section_text['EXPECTHEADERS'])) {
		...
		if ($resin_home) {
		  $old_php = $php;
		  $php = get_quercus_cgi_cmd();
		}
		else if (isset($php_cgi)) {
			$old_php = $php;
			$php = $php_cgi . ' -C ';
