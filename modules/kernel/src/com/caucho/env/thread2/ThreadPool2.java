...
public final class ThreadPool2 {
  ...
  private final ThreadIdleRing2 _idleRing
    = new ThreadIdleRing2();
  ...
  public static ThreadPool2 getThreadPool()
  {
    ThreadPool2 pool = _globalThreadPool.get();
    
    if (pool == null) {
      pool = new ThreadPool2();
  ...
  boolean execute(ResinThread2 thread)
  {
    ...
    else if (_taskQueue.schedule(thread)) {
      return true;
    }
    
    if (! _idleRing.offer(thread)) {
  ...
  class ThreadScheduleWorker extends AbstractTaskWorker2 {
    ...
    private boolean invoke()
    {
      if (! _priorityQueue.isEmpty()) {
        ResinThread2 thread = _idleRing.take();
