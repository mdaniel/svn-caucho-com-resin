...
import com.caucho.quercus.module.ProResinModuleContext;
 ...
 */
public class ProResinQuercus extends ResinQuercus implements ProfileQuercus
{
  ...
  protected ModuleContext createModuleContext(ModuleContext parent,
                                              ClassLoader loader)
  {
    return new ProResinModuleContext(parent, loader);
  }
  ...
  {
    return new ProSessionManager(this, _contextId);
  }
  ...
  protected void extendFunctionMap(String name, int id)
  {
    ...
    synchronized (_functionNameMap) {
      ...
      if (ns > 0) {
        globalId = findFunctionId(name.substring(ns + 1));
      }
  ...
  /*
   // XXX: profile functions need to be initialized when actually used
   // during normal initialization, they shouldn't be set (primarily
   // so regressions will run in a reasonable time.)
