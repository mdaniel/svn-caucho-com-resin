 ...
 * XXX: experimental
 */
public class PregMatchProxy
{
  private final PregMatchResult []_results = new PregMatchResult[5];
  ...
  public Value preg_match(Env env, Regexp regexp, StringValue subject,
                          Value matchRef, int flags, int offset)
  {
    if (_isPassthru)
      return RegexpModule.preg_match(env, regexp, subject, matchRef,
                                     flags, offset);
    ...
    for (int i = 0; i < _results.length; i++) {
      PregMatchResult result = _results[i];
      
      if (result == null) {
        Value val = RegexpModule.preg_match(env, regexp, subject, matchRef,
                                            flags, offset);
        
        _results[i] = new PregMatchResult(regexp, subject, matchRef,
                                          flags, offset, val);
  ...
  static class PregMatchResult
  {
    ...
    PregMatchResult(Regexp regexp, StringValue subject, Value matchRef,
