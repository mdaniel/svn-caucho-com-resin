 ...
 * parses afm
 */
public class AfmParserPdf {
  private static final L10N L = new L10N(AfmParserPdf.class);
   ...
   */
  public FontFacePdf parse(String name)
    throws IOException
  {
    String path = "com/caucho/pdf/font/" + name + ".afm";
    ...
    try (ReadStream is = Vfs.openRead(loader.getResourceAsStream(path))) {
      ...
      if (is == null) {
        throw new PdfException(L.l("{0} is an unknown font", name));
      }
  ...
  private FontFacePdf parseTop()
    throws IOException
  {
    FontFacePdf font = new FontFacePdf();
  ...
  private FontCharPdf parseCharacter()
    throws IOException
  {
    ...
    return new FontCharPdf(code, width);
  }
