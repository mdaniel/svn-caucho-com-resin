...
import com.caucho.v5.relaxng.program.EmptyItem;
  ...
  implements VerifierHandler
{
  ...
  private ArrayList<NameCfg> _nameStack = new ArrayList<NameCfg>();
    ...
    throws SAXException
  {
    ...
    try {
      ...
      for (int i = 0; i < len; i++) {
        ...
        name = new NameCfg(attrQName, attrUri);
  ...
  private Item getStartElement(Item item, NameCfg name)
    ...
    throws SAXException
  {
    ...
    NameCfg name = _name;
    NameCfg parent = _nameStack.remove(_nameStack.size() - 1);
    _name = parent;
   ...
   */
  private String errorNodeName(NameCfg name, Item item, Item parentItem)
  {
    ...
    HashSet<NameCfg> values = new LinkedHashSet<NameCfg>();
    currentItem.firstSet(values);

    for (NameCfg value : values) {
  ...
  private String errorMessageDetail(Item item, Item parentItem,
                                    String parentName, NameCfg qName)
  {
    Item currentItem = item;
   ...
   * Returns a string containing the allowed values.
   */
  private String requiredValues(Item item, String parentName, NameCfg qName)
  {
    if (item == null)
      return "";
  ...
  private String namesToString(HashSet<NameCfg> values,
                               String parentName,
                               NameCfg qName,
                               boolean allowEmpty)
  {
    ...
    if (values.size() > 0) {
      ArrayList<NameCfg> sortedValues = new ArrayList<NameCfg>(values);
      Collections.sort(sortedValues);
      
      for (int i = 0; i < sortedValues.size(); i++) {
        NameCfg name = sortedValues.get(i);
   ...
   * Returns a string containing the allowed values.
   */
  private String allowedAttributes(Item item, NameCfg qName)
  {
    if (item == null)
      return "";
  ...
  static class StartKey {
    ...
    public StartKey(Item item, NameCfg name)
    {
      _item = item;
      _name = name;
    }
    ...
    public void init(Item item, NameCfg name)
    {
      _item = item;
      _name = name;
    }
