...
import com.caucho.v5.relaxng.pattern.*;
...
public class RelaxBuilder extends DefaultHandler {
      ...
      if (RELAX_S.equals(uri)) {
        ...
        if (localName.equals("grammar")) {
          ...
          if (localName.equals("element")) {
            ...
            if (name != null) {
              NameCfg eltQName = getName(name);
              child.addNameChild(new NamePattern(eltQName));
            }

            if (name != null) {
              NameCfg attrQName = getName(name);
            ...
            if (name != null) {
              NameCfg attrQName = new NameCfg(name, _ns);
              child.addNameChild(new NamePattern(attrQName));
            }
          }
  ...
  private NameCfg getName(String name)
  {
    ...
    if (p > 0) {
      ...
      if (ns != null)
        return new NameCfg(name, ns);
      else
        return new NameCfg(name, _ns);
    }
    else
      return new NameCfg(name, _ns);
  }
