  ...
  implements ScanListenerByteCode, EnvironmentEnhancerListener
{
  ...
  private void parsePersistenceConfig(PathImpl root)
  {
    ...
    try {
      ...
      new ConfigXml().configure(persistence, is,
  ...
  private void registerPersistenceUnit(PersistenceUnitManager pUnit)
  {
    try {
      ...
      if (pUnit.getTransactionType() == PersistenceUnitTransactionType.JTA) {
        ...
        emFactory = beanManager.createBeanBuilder(EntityManager.class);
  ...
  */
  ...
  void addProviderUnit(ConfigPersistenceUnit unit)
  {
    try {
      ...
      if (cl == null) {
        throw new IllegalStateException(L.l("{0} has no available JPA providers",
                                            unit));
      }
