...
class Policy {
  ...
  NameCfg getAttributeName(CharBuffer eltName, CharBuffer source)
  {
   ...
   */
  NameCfg getAttributeName(CharBuffer eltName, CharBuffer source, boolean nsNull)
  {
    NameCfg qname = (NameCfg) _attrCache.get(source);
    if (qname != null)
      return qname;
    ...
    qname = new NameCfg(fullName, prefix, localName, ns);
   ...
   */
  NameCfg getName(CharBuffer source)
  {
    NameCfg qname = (NameCfg) nameCache.get(source);
    if (qname != null)
      return qname;
   ...
   * @return the QName including namespace for the source.
   */
  NameCfg getNamespaceName(CharBuffer source)
  {
    NameCfg qname = (NameCfg) nameCache.get(source);
    if (qname != null)
      return qname;
    ...
    qname = new NameCfg(prefix, localName, null); // XmlParser.XMLNS
   ...
   * @return the action code for the next node
   */
  int elementCloseAction(XmlParser parser, NameCfg node, String tagEnd)
    throws XmlParseException
  {
    String nodeName = node.getName();
