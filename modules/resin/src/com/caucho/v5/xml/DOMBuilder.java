...
public class DOMBuilder implements XMLWriter, ContentHandler, ErrorHandler {
  ...
  private ArrayList<NameCfg> _prefixNames = new ArrayList<NameCfg>();
  ...
  public void startPrefixMapping(String prefix, String url)
  {
    ...
    if (prefix == null || prefix.equals("")) {
      _prefixNames.add(new NameCfg(null, "xmlns", XmlParser.XMLNS));
      _prefixValues.add(url);
    }
    else {
      _prefixNames.add(new NameCfg("xmlns", prefix, XmlParser.XMLNS));
      _prefixValues.add(url);
    }
  }
  ...
  public void startElement(NameCfg name, QAttributes attributes)
    throws SAXException
  {
    popText();
    ...
    for (int i = 0; i < _prefixNames.size(); i++) {
      NameCfg attrName = _prefixNames.get(i);
    ...
    for (int i = 0; i < length; i++) {
      NameCfg attrName = attributes.getName(i);
