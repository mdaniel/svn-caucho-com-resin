...
import com.caucho.v5.server.admin.AdminService;
...
public class CdiExtensionResin implements Extension {
  ...
  private <T> void processBeanImpl(ProcessBean<T> event)
  {
    ...
    */
    ...
    if (annotated.isAnnotationPresent(Jndi.class)) {
      ...
      try {
        com.caucho.v5.naming.JndiUtil.bindDeepShort(jndiName, proxy);
      } catch (Exception e) {
                                   ...
                                   Bean<?> bean)
  {
    ...
    try {
      com.caucho.v5.naming.JndiUtil.bindDeepShort("java:module/" + jndiName, proxy);
      com.caucho.v5.naming.JndiUtil.bindDeepShort(jndiName, proxy);
    } catch (Exception e) {
      log.log(Level.FINE, e.toString(), e);
    }
