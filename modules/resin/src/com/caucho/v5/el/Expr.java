...
public abstract class Expr extends ValueExpression {
  ...
  final static int MATCHES = COND_BINARY + 1;
  final static int ASSIGN = MATCHES + 1;
  final static int SEMICOLON = ASSIGN + 1;
  final static int CONCAT = SEMICOLON + 1;
  final static int LAMBDA = CONCAT + 1;

  final static int IDENTIFIER = LAMBDA + 1;
...
//      return ((LongLiteral)value).evalLong(env);
//    }
//    else if (value instanceof DoubleLiteral) {
//      return ((DoubleLiteral)value).evalLong(env);
//    }
    else {
    ...
    throws ELException
  {
    ...
    if (obj == null && (! targetType.isPrimitive() &&
