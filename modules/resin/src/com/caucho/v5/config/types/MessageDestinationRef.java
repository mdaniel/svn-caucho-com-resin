  ...
  implements ObjectFactoryNaming
{
    ...
    throws NamingException
  {
    ...
    String fullRefName = JndiUtil.getFullName(_refName);

    if (_link != null) {
      bind = true;
    }
    else if (_foreignName != null) {
      String fullForeignName = JndiUtil.getFullName(_foreignName);
    ...
    if (bind) {
      JndiUtil.rebindDeep(fullRefName, this);
    }
    ...
    throws NamingException
  {
    ...
    if (_context != null) {
      target = _context.lookup(JndiUtil.getFullName(foreignName));
    }
    else
      target = JndiUtil.lookup(foreignName);
