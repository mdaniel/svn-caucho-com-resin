...
@Module
public class IntrospectedBeanBase<T> extends BeanBase<T>
  implements PassivationCapable, PassivationSetter
{
  private static final L10N L = new L10N(IntrospectedBeanBase.class);
  private static final Logger log
    = Logger.getLogger(IntrospectedBeanBase.class.getName());
  ...
  private ArrayList<Class<? extends Annotation>> _stereotypes
    = new ArrayList<>();
  ...
  public IntrospectedBeanBase(CandiManager manager,
                                  Type type,
                                  Annotated annotated,
                                  BeanAttributes<T> beanAttributes)
  {
    ...
    if (beanAttributes != null) {
      _scope = beanAttributes.getScope();
      _name = beanAttributes.getName();
      _isAlternative = beanAttributes.isAlternative();

      _stereotypes = new ArrayList<>(beanAttributes.getStereotypes());
      _qualifiers = new ArrayList<>(beanAttributes.getQualifiers());
      _typeClasses = new HashSet<>(beanAttributes.getTypes());
    }
  ...
  public Set<Class<? extends Annotation>> getStereotypes()
  {
    Set<Class<? extends Annotation>> set
      = new LinkedHashSet<Class<? extends Annotation>>(_stereotypes);
...
*/
  ...
  protected void introspect(Annotated annotated)
  {
    ...
    if (_isAlternative != null) {
    }
    else if (annotated.isAnnotationPresent(Alternative.class)) {
      // ioc/0618
      _isAlternative = true;
    }
    else {
  ...
  protected void introspectScope(Annotated annotated)
  {
    ...
    BeanManager inject = getInjectManager();
  ...
  protected void introspectStereotypes(Annotated annotated)
  {
    ...
    for (Annotation stereotype : annotated.getAnnotations()) {
      ...
      Set<Annotation> stereotypeSet =
        getInjectManager().getStereotypeDefinition(stereotypeType);
      ...
      for (Annotation ann : stereotypeSet) {
        ...
        if (annType.isAnnotationPresent(Scope.class)
          ...
          if (_scope == null && scope != null && ! scope.equals(annType)) {
            throw new DefinitionException(L.l("'{0}' is an invalid @Scope because a scope '{1}' has already been defined.  Only one @Scope or @NormalScope is allowed on a bean.",
        ...
        if (annType.equals(Named.class) && _name == null) {
          ...
          if (! "".equals(namedValue))
            throw new DefinitionException(L.l("@Named must not have a value in a @Stereotype definition, because @Stereotypes are used with multiple beans."));
        }
  ...
  {
    EventManager eventManager = getInjectManager().getEventManager();
  ...
  protected void introspectSpecializes(Annotated annotated)
  {
    ...
    */
    ...
    if (baseClass.getSuperclass() == null ||
      ...
      throw new DefinitionException(L.l("{0}: invalid @Specializes bean because the superclass '{1}' is not a managed bean.",
  ...
  protected void validateSpecializes(Annotated annotated, Class<?> parentClass)
  {
  }
  ...
  {
    return getInjectManager().isNormalScope(getScope());
  }
