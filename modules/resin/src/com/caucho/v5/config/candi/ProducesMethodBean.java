 ...
 */
@Module
public class ProducesMethodBean<X,T> extends IntrospectedBeanBase<T>
  implements ScopeAdapterBean<X>, ProducerFactory<X>
{
  ...
  private ProducesMethodBean(CandiManager manager,
                             Bean<X> producerBean,
  ...
  {
    super(manager, producesMethod.getBaseType(), producesMethod, beanAttributes);
    
    // Objects.requireNonNull(producerBean);
    Objects.requireNonNull(producesMethod);
    Objects.requireNonNull(producesArgs);
    ...
    if (Object.class.equals(retType)
        && ! javaMethod.isAnnotationPresent(ProxyProduces.class)) {
      throw new IllegalArgumentException(L.l("Producer method must specify a concrete return type: {0}",
                                             javaMethod));
    }
         ...
         Arg<? super X> []disposesArgs)
  {
    BeanAttributes attributes
      = new BeanAttributesImpl(producesMethod, manager);

    attributes = manager.processBeanAttributes(attributes, producesMethod);
  ...
  private void introspectInjectionPoints()
  {
    for (AnnotatedParameter<?> param : _producesMethod.getParameters()) {
      InjectionPointImpl ip = new InjectionPointImpl(getInjectManager(), this, param);
  ...
  public X getScopeAdapter(Bean<?> topBean, CreationalContextImpl<X> cxt)
  {
    ...
    if (scopeType != null) {
      ...
      if (value == null) {
        ...
        _scopeAdapter = scopeAdapter.wrap(getInjectManager().getBeanManager().createNormalInstanceFactory(topBean));
        value = _scopeAdapter;
      }
  ...
  class MethodProducer implements Producer<T> {
    ...
    public T produce(CreationalContext<T> cxt)
    {
      ...
      Bean<X> producerBean = _producerBean;
      
      ProducesCreationalContext<X> factoryEnv = null;
      
      X factory = CreationalContextImpl.find(env, producerBean);
      
      if (factory == null && _producerBean != null) {
        factoryEnv = new ProducesCreationalContext<X>(producerBean, env);
        
        factory = getInjectManager().getReference(producerBean, factoryEnv);
      ...
      if (env != null
          && producerBean != null 
          && producerBean.getScope() == Dependent.class) {
        factoryEnv.release();
