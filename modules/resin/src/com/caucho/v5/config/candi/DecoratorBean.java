 ...
 */
@Module
public class DecoratorBean<T> implements Decorator<T>, WithBeanManager
{
  ...
  private DecoratorBean(CandiManager manager,
                        Bean<T> bean,
                        Class<T> selfType)
  {
    _selfType = selfType;
    ...
    _beanManager = manager.getBeanManager(bean);
  }

  static <T> DecoratorBean<T> create(CandiManager manager,
                                     Bean<T> bean)
  {
    DecoratorBean decorator = new DecoratorBean(manager, bean, bean.getBeanClass());
  ...
  {
    AnnotatedType<T> annotated = manager.createAnnotatedType(type);

    Bean<T> cdiBean = manager.createManagedBean(annotated, attributes);

    Bean<T> injectedBean = new FactoryInjectedBean<T>(cdiBean, factory);

    DecoratorBean decoratorBean = new DecoratorBean(manager, injectedBean, type);
  ...
  public void bind()
  {
    ...
    if (_delegateField == null
      ...
      throw new DeploymentException(L.l("{0} is missing a @Delegate field.  All @Decorators need a @Delegate field for a delegate injection",
                                        _selfType.getName()));
    
    if (_selfType.isAnnotationPresent(Interceptor.class))
      throw new ConfigException(L.l("{0} is an invalid @Delegate because it has an @Interceptor annotation.",
                                    _selfType.getName()));
    
    if (Modifier.isFinal(_selfType.getModifiers())) {
  ...
  protected void introspect()
  {
    ...
    for (InjectionPoint ip : _bean.getInjectionPoints()) {
      if (ip.isDelegate()) {
        if (_delegateInjectionPoint != null)
          throw new DefinitionException(L.l("{0}: @Decorator field '{1}' conflicts with earlier field '{2}'."
    ...
    if (_delegateInjectionPoint != null) {
      ...
      _typeSet = new LinkedHashSet<>();
      
      for (Type type : selfType.getTypeClosure(manager.getTypeFactory())) {
        ...
        if (baseType.isAssignableFrom(delegateType)) {
          ...
          throw new DefinitionException(L.l("{0}: '{1}' is a Decorator type not implemented by the delegate {2}",
  ...
  public String toString()
  {
    ...
    sb.append("[");
    sb.append(_selfType.getSimpleName());
