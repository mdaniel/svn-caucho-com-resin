...
import com.caucho.v5.config.reflect.BaseType.ClassFill;
  ...
  implements AnnotatedType<T>
{
  ...
  ReflectionAnnotatedType(CandiManager manager, 
                          BaseType type)
  {
    super(type,
          type.getTypeClosure(manager.getTypeFactory()),
    ...
    synchronized (this) {
      if (! _isIntrospected) {
        try {
          ...
          throw new ReflectionException(L.l("NoClassDefFoundError {0} while introspecting '{1}'",
                                             e.getMessage(), _javaClass), e);
        }
  ...
  private void introspect(Class<T> cl)
  {
    try {
      ...
      if (! cl.isInterface()) {
        for (Constructor<?> ctor : AnnotatedTypeUtil.getDeclaredConstructors(cl)) {
  ...
  private void introspectFields(Class<?> cl)
  {
    ...
    for (Field field : AnnotatedTypeUtil.getDeclaredFields(cl)) {
      try {
        // ioc/0p23
                                 ...
                                 HashMap<String,BaseType> paramMap)
  {
    ...
    for (Method method : AnnotatedTypeUtil.getDeclaredMethods(cl)) {
  ...
  private void introspectSpecializesAnnotations(Class<?> cl)
  {
    ...
    if (cl.isAnnotationPresent(Named.class))
      throw new DefinitionException(L.l("'{0}' is an invalid @Specializes bean because it has a @Named annotation.",
                                        cl.getName()));
