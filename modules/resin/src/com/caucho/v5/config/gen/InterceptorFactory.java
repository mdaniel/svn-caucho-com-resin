...
import com.caucho.v5.config.candi.AnnotatedOverrideMap;
  ...
  extends AbstractAspectFactory<X>
{
  ...
  private static final Logger log
    = Logger.getLogger(InterceptorFactory.class.getName());
  ...
  protected boolean isStateful(Annotation ann)
  {
    return false;
  }
  ...
  {
    for (Method m : getDeclaredMethods(cl)) {
  ...
  private Method []getDeclaredMethods(Class<?> cl)
  {
    try {
  ...
  private void introspectClassDecorators()
  {
    ...
    BeanManagerBase beanManager = _manager.getBeanManager(getBeanType());

    List<Decorator<?>> decorators
      = beanManager.resolveDecorators(types, decoratorBindings);

    // _manager.filterDecorators(decorators, getBeanType().getJavaClass());
    ...
    for (Decorator<?> decorator : decorators) {
      ...
      closure.addAll(type.getTypeClosure(_manager.getTypeFactory()));
    }
                         ...
                         Annotation ann)
  {
    ...
    if (annType.isAnnotationPresent(InterceptorBinding.class)) {
      ...
      if (oldAnn != null && ! oldAnn.equals(ann))
        throw new DefinitionException(L.l("duplicate @InterceptorBindings {0} and {1} are not allowed, because CDI can't tell which one to use.",
                                          ann, oldAnn));
    }
