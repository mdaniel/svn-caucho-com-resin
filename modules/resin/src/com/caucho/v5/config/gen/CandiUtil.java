...
import com.caucho.v5.config.candi.DecoratorBean;
import com.caucho.v5.config.candi.DelegateProxyBean;
import com.caucho.v5.config.candi.DependentCreationalContext;
import com.caucho.v5.config.candi.InterceptorRuntimeBean;
...
public class CandiUtil {
  ...
  public static int []createInterceptors(BeanManagerBase manager,
                                         ...
                                         Annotation ...bindings)
  {
    ...
    */
    ...
    if (bindings != null && bindings.length > 0) {
      ...
      // manager.filterInterceptors(interceptors, interceptedClass);
    }
    else
    ...
    for (int i = 0; i < interceptors.size(); i++) {
      ...
      if (index >= 0 && interceptor instanceof InterceptorRuntimeBean) {
        InterceptorRuntimeBean bean = (InterceptorRuntimeBean) interceptor;

        while (bean != null && !bean.interceptsDirect(type))
          bean = bean.getParent();

        index = bean == null ? index : beans.indexOf(bean);
      }

      if (index >= 0)
        indexList.add(index);
      else if (interceptor instanceof InterceptorRuntimeBean) {
        addStaticBean((InterceptorRuntimeBean<?>) interceptor,
                      indexList,
                      beans,
                      type);
      } else {
                ...
                InterceptionType type)
  {
    ...
    if (staticBean.interceptsDirect(type)) {
      int index = beans.indexOf(staticBean);
      indexList.add(index);
    }
  }

  public static void createInterceptors(CandiManager manager,
                                        ArrayList<Interceptor<?>> beans,
                                        Annotation ...bindings)
  {
    if (bindings == null || bindings.length == 0)
                                         ...
                                         String typeName)
  {
    ...
    if (! Serializable.class.isAssignableFrom(beanClass)) {
      DeploymentException exn
        = new DeploymentException(L.l("{0}: {1} is an invalid {2} because it is not serializable.",
                                      cl.getName(),
                                      bean,
                                      typeName));
    ...
    for (InjectionPoint ip : bean.getInjectionPoints()) {
      ...
      if (! Serializable.class.isAssignableFrom(type)) {
        DeploymentException exn
          = new DeploymentException(L.l("{0}: {1} is an invalid {4} because its injection point '{2}' of type {3} is not serializable.",
                                        cl.getName(),
                                        bean,
    ...
    for (int i = 0; i < decorators.size(); i++) {
      ...
      try {
        ...
        if (Modifier.isAbstract(method.getModifiers()))
          method = null;
/*
        if (AbstractClassPadding.class.isAssignableFrom(beanClass)
            && beanClass.equals(method.getDeclaringClass()))
          method = null;
*/
        ...
        methods[decorators.size() - i - 1] = method;
      } catch (Exception e) {
  ...
  public static Method findDeclaredMethod(Class<?> cl,
                                          String methodName,
                                          Class<?> ...paramTypes)
  {
  ...
  {
    Method match = findAccessibleMethod(method.getDeclaringClass(),
                                        method.getName(),
                                            ...
                                            Class<?> ...paramTypes)
  {
    ...
    // ioc/0c2c
    Method method = findDeclaredMethod(cl, methodName, paramTypes);
    if (method != null) {
      method.setAccessible(true);
      return method;
    }
    
    method = findAccessibleMethod(cl.getSuperclass(), methodName, paramTypes);
    ...
    for (Class<?> api : cl.getInterfaces()) {
      method = findAccessibleMethod(api, methodName, paramTypes);
  ...
  public static Object []generateProxyDelegate(CandiManager manager,
                                               List<Decorator<?>> beans,
