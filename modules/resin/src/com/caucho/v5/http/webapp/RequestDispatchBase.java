...
import com.caucho.v5.http.protocol.ServletInputStreamImpl;
 ...
 */
class RequestDispatchBase extends RequestAdapter
{
  protected static final Logger log
    = Logger.getLogger(RequestDispatchBase.class.getName());
   ...
   */
  public static RequestDispatchBase createDispatch()
  {
    RequestDispatchBase req = _freeList.allocate();
    if (req == null)
      req = new RequestDispatchBase();
  ...
  public ReadStream getStream()
    throws IOException
  {
    if (_readStream == null && getRequest() instanceof RequestCaucho)
    ...
    throws IOException
  {
    ...
    if (_is == null) {
      _is = new ServletInputStreamImpl(null);
    }
  ...
  private SessionImpl createSession(boolean create, boolean hasOldSession)
  {
    ...
    if (id != null && id.length() > 6) {
      ...
      if (session == null) {
        ...
        if (session != null)
          setHasCookie();
        if (! session.getId().equals(id) && manager.isEnableSessionCookies()) {
          HttpServletResponse response = getResponse();

          if (response instanceof ResponseCaucho)
            ((ResponseCaucho) getResponse()).setSessionId(session.getId());
        }
    ...
    if (manager.isEnableSessionCookies()) {
      HttpServletResponse response = getResponse();
  ...
  public boolean isLoginRequested()
  {
    if (! (getRequest() instanceof RequestCaucho))
      ...
      return ((RequestCaucho) getRequest()).isLoginRequested();
  }
  ...
  public boolean login(boolean isFail)
  {
    if (! (getRequest() instanceof RequestCaucho))
      ...
      return ((RequestCaucho) getRequest()).login(isFail);
  }
   ...
   * Frees the request.
   */
  public static void free(RequestDispatchBase req)
  {
    req.free();
