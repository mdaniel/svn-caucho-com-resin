...
import com.caucho.v5.java.ScriptStackTrace;
...
public class ErrorPageManager
{
   ...
   */
  public ErrorPageManager(HttpContainerServlet server)
  {
    ...
    throws IOException
  {
    ...
    if (req != null && req.isAsyncStarted()) {
    ...
    if (response instanceof ResponseServlet) {
      ResponseServlet resFacade = (ResponseServlet) response;
      resFacade.killCache();
    ...
    if (isBadRequest) {
      ...
      if (request instanceof RequestCaucho)
        ((RequestCaucho) request).killKeepalive("bad request: " + rootExn);
    ...
    */
    ...
    if (isDevelopmentModeErrorPage()) {
      ...
      else  if (rootExn instanceof DisplayableException) {
        out.println(escapeHtml(rootExn.getMessage()));
      }
      else {
  ...
  private void printVersion(PrintWriter out)
    throws IOException
  {
    HttpContainerServlet server = _server;
    String version = null;
    ...
    if (version != null) {
      ...
      if (server != null)
        out.println("Server: '" + server.getServerDisplayName() + "'");
  ...
  private String getServletName(ServletRequest request)
  {
    if (request instanceof RequestServlet)
      return ((RequestServlet) request).getServletName();
   ...
   */
  public void sendError(RequestCaucho request,
                        ResponseCaucho response,
   ...
   * @param message a string message
   */
  public void sendErrorImpl(RequestCaucho request,
                            ResponseCaucho response,
                            int code, String message)
    throws IOException
  {
   ...
   */
  private boolean handleErrorStatus(RequestCaucho request,
                                    ResponseCaucho response,
                                    int code, String message)
