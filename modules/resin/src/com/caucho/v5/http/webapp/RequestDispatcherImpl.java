...
public class RequestDispatcherImpl implements RequestDispatcher {
  ...
  private InvocationServlet _dispatchInvocation;
  private InvocationServlet _requestInvocation;
  private InvocationServlet _asyncInvocation;
  private boolean _isLogin;

  RequestDispatcherImpl(InvocationServlet includeInvocation,
                        InvocationServlet forwardInvocation,
  ...
  public InvocationServlet getAsyncInvocation()
  {
    if (_asyncInvocation == null) {
      ...
      chain = new FilterChainResume(chain, invocation.getWebApp());
  ...
  public void dispatch(ServletRequest request, ServletResponse response)
    throws ServletException, IOException
  {
    if (request.getServletContext() instanceof WebAppUnknown) {
  ...
  public void forward(ServletRequest topRequest, ServletResponse topResponse,
                      String method, InvocationServlet invocation,
                      DispatcherType type)
    throws ServletException, IOException
  {
    ResponseCaucho cauchoRes = null;

    boolean isAllowForward = false;
    
    if (_webApp != null)
      isAllowForward = _webApp.isAllowForwardAfterFlush();

    if (topResponse instanceof ResponseCaucho) {
      cauchoRes = (ResponseCaucho) topResponse;

      cauchoRes.setForwardEnclosed(! isAllowForward);
    }

    // jsp/15m8
    if (topResponse.isCommitted() && method == null && ! isAllowForward) {
      ...
      if (cauchoRes != null) {
        ...
        while (resp != null) {
          if (isAllowForward && resp instanceof ResponseInclude) {
            // server/10yh
            break;
          }
          else if (resp instanceof ResponseCaucho) {
    ...
    RequestForward subRequest;

    if (_isLogin)
      subRequest = new RequestLogin(parentReq, parentRes, invocation);
    else if (type == DispatcherType.ERROR)
      subRequest = new RequestError(parentReq, parentRes, invocation);
    else if (type == DispatcherType.REQUEST)
      subRequest = new RequestDispatch(parentReq, parentRes, invocation);
    else
      subRequest = new RequestForward(parentReq, parentRes, invocation);
    ...
    ResponseForward subResponse = subRequest.getResponse();
    ...
    try {
      ...
      invocation.service((RequestServlet) topRequest, (ResponseServlet) topResponse);
    ...
    throws ServletException, IOException
  {
    if (_webApp.isAllowForwardAfterFlush()) {
      ...
      if (res instanceof ResponseCaucho) {
        ResponseCaucho cauchoResponse = (ResponseCaucho) res;
  ...
  public void include(ServletRequest topRequest, ServletResponse topResponse,
    ...
    throws ServletException, IOException
  {
    InvocationServlet invocation = _includeInvocation;
    ...
    RequestInclude subRequest
      = new RequestInclude(parentReq, parentRes, invocation);
    
    // server/10yf, jsp/15di
    if (subRequest.getRequestDepth(0) > MAX_DEPTH)
      throw new ServletException(L.l("too many servlet includes '{0}'", parentReq.getServletPath()));

    ResponseInclude subResponse = subRequest.getResponse();
    ...
    throws ServletException, IOException
  {
    ...
    RequestServlet bottomRequest = null;
    ResponseServlet bottomResponse = null;
    ...
    bottomRequest = (RequestServlet) req;
    ...
    bottomResponse = (ResponseServlet) res;
    
    RequestAsync asyncRequest
      = new RequestAsync(bottomRequest, bottomResponse, invocation);
    ...
    try {
      invocation.service((RequestServlet) request, (ResponseServlet) response);
    } finally {
      // subRequest.finishRequest();
