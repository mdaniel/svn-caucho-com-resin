...
{
  public enum FragmentMode {IN_FRAGMENT, IN_WEBXML};

  private static L10N L = new L10N(ServletBuilder.class);
  ...
  //XXX: replace with a strategy pattern
  //private Bean _bean;
  ...
  public ServletBuilder(FragmentMode fragmentMode)
  {
  ...
  public ServletBuilder createRegexpConfig(String servletName)
  ...
  public Set<String> setServletSecurity(ServletSecurityElement securityElement)
  {
    ...
    ConstraintManager manager = getWebApp().getConstraintManager();
    
    manager.addSecurityElement(getServletName(), securityElement);
  ...
  public Set<String> addMapping(String... urlPatterns)
  {
    ...
    try {
      ...
      ServletMapping mapping = _webApp.getBuilder().createServletMapping();
  ...
  {
    _bean = bean;
    _servletClass = bean.getBeanClass();
  }
  */
  ...
  */
  ...
  public void setServletClass(String servletClassName)
  {
    ...
    try {
     ...
     // ioc/0p2b vs servlet/1177

      /*
      WebServlet webServlet = _servletClass.getAnnotation(WebServlet.class);
      
      if (webServlet != null && webServlet.asyncSupported()) {
        _isAsyncSupported = true;
      }
      */
      ...
      try {
        ...
        if (e instanceof ClassNotFoundException && e.getCause() instanceof CompileException) {
          msg = e.toString();
                ...
                && envLoader != null
                && envLoader.getConfigException() != null) {
          msg = L.l("'{0}' is not a known servlet class.  Servlets belong in the classpath, for example WEB-INF/classes.\n  {1}",
                    _servletClassName,
                    envLoader.getConfigException().getMessage());
  ...
  {
    if (! _initParams.containsKey(param)) {
  ...
  public void setInitParam(InitParam initParam)
  {
    if (_webApp.isAllowInitParamOverride())
      ...
      for (Map.Entry<String, String> param
            : initParam.getParameters().entrySet()) {
        if (! _initParams.containsKey(param.getKey())) {
          _initParams.put(param.getKey(), param.getValue());
  ...
  {
    _fragmentMode = FragmentMode.IN_FRAGMENT;
  }

  public boolean isInFragmentMode()
  {
    return _fragmentMode == FragmentMode.IN_FRAGMENT;
  }
  ...
  public void merge(ServletBuilder config) 
  {
    if (_loadOnStartup == Integer.MIN_VALUE) {
    ...
    throws ServletException
  {
    ...
    if (_servletClassName == null) {
      ...
      if (_servletClass != null) {
        ...
        throw error(L.l("'{0}' is not a known servlet class.  Servlets belong in the classpath, for example WEB-INF/classes.{1}",
                        _servletClassName, subMsg));
      }
      else {
      ...
      */
      ...
      ServletSecurity security
        = _servletClass.getAnnotation(ServletSecurity.class);
      
      if (security != null && ! getWebApp().getBuilder().isMetadataComplete()) {
        introspectSecurity(security);
      }
  ...
  private void introspectSecurity(ServletSecurity security)
  {
    setServletSecurity(new ServletSecurityElement(security));
  }
  ...
  public void handleAlarm(Alarm alarm)
  {
    try {
      ...
      ServletRequest req = new RequestServletStub();
      ServletResponse res = new ResponseServletStub();
    ...
    throws ServletException
  {
    ...
    if (_singletonServlet != null) {
      servletChain = new FilterChainServlet(this);
    ...
    return createServletChain(servletClass);
  }
    ...
    throws ServletException
  {
    if (SingleThreadModel.class.isAssignableFrom(servletClass)) {
      return new FilterChainSingleThreadServlet(this);
    }
    else {
      return new FilterChainServlet(this);
    }
  }
  ...
  protected Object createServlet(boolean isNew)
    throws ServletException
  {
    // server/102e
  ...
  private boolean isMXBean(Object servlet)
  {
    return false;
  }
  ...
  */
  ...
  protected Object createServletImpl()
    throws Exception
  {
    ...
    throws Exception
  {
    ...
    if (bean != null) {
      ...
      CreationalContextImpl env = new OwnerCreationalContext(bean);
      
      // server/5130
      Object value = injectManager.findReference(bean);
    ...
    */
    ...
    if (servletClass != null) {
      ...
      //try {
        // server/1b40
        /*
        _injectionTarget = injectManager.createInjectionTarget(servletClass);

        _servletEnv = new OwnerCreationalContext(null);

        servlet = _injectionTarget.produce(_servletEnv);

        _injectionTarget.inject(servlet, _servletEnv);
        */
        servlet = injectManager.lookup(servletClass);
        // server/47b1
        /*
  ...
  /*
  protected Bean<?> resolveBean()
  {
    if (_bean == null) {
      CandiManager cdiManager = getBeanManager();

      Class<?> servletClass = getServletClass();

      if (servletClass != null) {
        // ioc/0p16, ioc/0p19 vs hudson/0009
        //_bean = cdiManager.createTransientBean(servletClass);
        
        Set beans = cdiManager.getBeans(servletClass);
  ...
  */
  ...
  void killServlet()
  {
    ...
    /*
    if (_bean != null) {
      _bean.destroy(servlet, _servletEnv);
    }
    else if (_injectionTarget != null) {
      _injectionTarget.preDestroy(servlet);
    }
    */
  ...
  protected void copyFrom(ServletBuilder source)
  {
