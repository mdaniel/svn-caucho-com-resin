...
public class FormLoginServlet extends GenericServlet {
    ...
    throws ServletException, IOException
  {
    ...
    try {
      login = getFormLogin(webApp.getLogin());
    ...
    if (user == null) {
      ...
      if (res instanceof ResponseCaucho) {
        ((ResponseCaucho) res).killCache();
        ((ResponseCaucho) res).setNoCache(true);
      }
      else {
    ...
    if (uri == null) {
      log.warning(L.l("FormLogin: session saved uri no longer exist in session '{0}'",
