 ...
 *
 * @see com.caucho.v5.jsp.AnalyzedTag
 */
public class TagAnalyzer
{
  ...
  private static final Class<? extends Annotation> _classPersistenceContext;
  private static final Class<? extends Annotation> _classPersistenceUnit;
    ...
    */
    ...
    for (Method method : cl.getDeclaredMethods()) {
      ...
      if (method.getName().startsWith("set")
          && (method.isAnnotationPresent(Resource.class)
              ...
              || isAnnotationPresent(method, _classPersistenceContext)
              || isAnnotationPresent(method, _classPersistenceUnit))) {
        tag.setHasInjection(true);
      }
    ...
    for (Field field : cl.getDeclaredFields()) {
      if (field.isAnnotationPresent(Resource.class)
          ...
          || isAnnotationPresent(field, _classPersistenceContext)
          || isAnnotationPresent(field, _classPersistenceUnit)) {
        tag.setHasInjection(true);
      }
    }
  ...
  {
    return cl != null && method.isAnnotationPresent(cl);
  }
  
  private boolean isAnnotationPresent(Field field, 
                                      Class<? extends Annotation> cl)
  {
    return cl != null && field.isAnnotationPresent(cl);
  }
  ...
  static {
    Class classPersistenceContext = null;
    
    try {
      classPersistenceContext = Class.forName("javax.persistence.PersistenceContext");
    } catch (ClassNotFoundException e) {
      log.finest(e.toString());
    }
    
    Class classPersistenceUnit = null;
    
    try {
      classPersistenceUnit = Class.forName("javax.persistence.PersistenceUnit");
    } catch (ClassNotFoundException e) {
      log.finest(e.toString());
    }
    
    _classPersistenceContext = classPersistenceContext;
    _classPersistenceUnit = classPersistenceUnit;
  }
}
