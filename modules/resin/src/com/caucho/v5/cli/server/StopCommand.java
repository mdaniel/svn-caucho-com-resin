 ...
 */
public class StopCommand extends ServerCommandBase<ArgsDaemon>
{
  private static Logger log = Logger.getLogger(StopCommand.class.getName());
  private static L10N L = new L10N(StopCommand.class);
    ...
    throws BootArgumentException
  {
    ...
    try (ClientWatchdog client = new ClientWatchdog(args, server)) {
      ...
      watchdog.stop(server.getPort(), ShutdownModeAmp.GRACEFUL);

      System.out.println(L.l(
        "{0}/{1} stopped --port {2} for watchdog at {3}:{4}",
        args.getDisplayName(),
        Version.getVersion(),
        server.getPort(),
      ...
      System.out.println(L.l(
        "{0}/{1} can't stop --port {2} for watchdog at {3}:{4}.\n{5}",
        args.getDisplayName(),
        Version.getVersion(),
        server.getPort(),
        server.getWatchdogAddress(args),
  ...
  private boolean stopBackground(ArgsDaemon args, ServerConfigBoot serverConfig)
  {
    ...
    System.out.println("  stop --port " + serverConfig.getPort() + " (background)");
