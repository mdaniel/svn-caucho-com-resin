 ...
 */
public class EjbManyToManyMethod extends CmpGetter {
  private static final L10N L = new L10N(EjbManyToManyMethod.class);

  private CmrManyToMany _manyToMany;
  ...
  {
    String listName = _manyToMany.getName() + "_List";

    CollectionClass list = new CollectionClass(_manyToMany, listName);
  ...
  class BeanMethod extends BaseMethod {
      ...
      throws IOException
    {
      ...
      String abstractSchema = _manyToMany.getBean().getAbstractSchemaName();
      
      out.print("String sql = \"SELECT o." + _manyToMany.getName());
      out.print(" FROM " + abstractSchema + " o");
      out.print(" WHERE ");

      SelfEntityType type = _manyToMany.getBean().getEntityType();
	...
	out.print("o." + key.getName() + "=?" + (i + 1));
      ...
      EjbConfig config = _manyToMany.getBean().getConfig();
	...
	AbstractQueryMethod.generateSetParameter(out,
      ...
      out.print(listName + " list = ");
      
      out.println("new " + listName + "(this, query);");
