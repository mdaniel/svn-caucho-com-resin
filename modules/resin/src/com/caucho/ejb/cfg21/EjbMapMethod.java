...
import com.caucho.ejb.gen21.MapClass;
...
public class EjbMapMethod extends CmpGetter {
   ...
   */
  public EjbMapMethod(EjbView view,
  ...
  {
    String mapName = _map.getName() + "_Map";
    
    MapClass map = new MapClass(_map, mapName);
    
    beanAssembler.addComponent(map);
  ...
  class BeanMethod extends BaseMethod {
	...
	if (key.getName().equals(id)) {
      ...
      out.print("String sql = \"SELECT o." + _map.getIndexName() + ", o");
      out.print(" FROM " + abstractSchema + " o");
      out.print(" WHERE ");

      SelfEntityType targetType = _map.getBean().getEntityType();
      ArrayList<IdField> keys = targetType.getId().getKeys();
	...
	out.print("o." + id + "." + key.getName() + "=?" + (i + 1));
      }
	...
	AbstractQueryMethod.generateSetParameter(out,
      ...
      out.print(mapName + " map = ");
      
      out.println("new " + mapName + "(this, query);");
